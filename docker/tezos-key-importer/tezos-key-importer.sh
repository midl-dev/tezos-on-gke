...
/usr/local/bin/tezos-client -p $PROTOCOL_SHORT --base-dir $client_dir import secret key k8s-baker-${CUSTOMER_NAME} unencrypted:$INSECURE_PRIVATE_BAKING_KEY -f
else
    ...
    exec "${bin_dir}/tezos-client" --base-dir $client_dir -p $PROTOCOL_SHORT import secret key k8s-baker-${CUSTOMER_NAME} http://${KUBERNETES_NAME_PREFIX}-tezos-remote-signer-loadbalancer-${CUSTOMER_NAME}:8445/$PUBLIC_BAKING_KEY -f
fi
